...
class Game:
    ...
    def heuristic(self):
        score = 0
        for fantom in self.game_state['characters']:
            if (fantom['suspect'] == False):
                continue
            reveal = True
            if (self.x[fantom['position']] > 1 and fantom['position'] != self.game_state['shadow']):
                reveal = False
            ...
            for c in data:
                ...
                for position in self.getDestinations(charact):
                    ...
                    for powerUseNb in range(0, self.powerLoopNb(charact, testIdx) + 1):
                        ...
                        if (depth == maxDepth and nb == -1000000000):
